...
import ReturnProductsTable from '@/components/Return/ReturnProductsTable';
import AddProductForm, { AddProductFormRef } from '@/components/Return/AddProductForm';
import CloseReturnModal from '@/components/Return/CloseReturnModal';
import ReturnCloseResultModal from '@/components/Return/ReturnCloseResultModal';
...
export default function ReturnDetailsPage() {
  ...
  const [products, setProducts] = useState<ReturnDetail[]>([]);
  ...
  const [productToEdit, setProductToEdit] = useState<ReturnDetail | null>(null);
  ...
  const fetchReturn = async () => {
    try {
      ...
      router.push(`/${locale}/dashboard/almacenes/devoluciones`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const response = await returnService.getReturnDetails(returnId, 1);
  ...
  useEffect(() => {
    if (!initialFetchDone.current && returnId && can(['return_module_view'])) {
  ...
  const handleAddProductFormSuccess = () => {
    ...
    fetchReturn(); // Recargar los datos de la devolución
  ...
  const handleAddProductSave = async () => {
    if (!productFormRef.current || !returnItem) return;
    ...
    try {
      ...
      if (formData) {
        await returnService.addProductToReturn(returnItem.id, formData);
  ...
  const handleEditProductSave = async () => {
    if (!productFormRef.current || !returnItem || !productToEdit) return;
    ...
    try {
      ...
      if (formData) {
        await returnService.updateReturnDetail(returnItem.id, productToEdit.id, formData);
  ...
  const handleDeleteProduct = async (detailId: string) => {
    ...
    try {
      await returnService.deleteReturnDetail(returnItem.id, detailId);
  ...
  const handleEditProduct = (product: ReturnDetail) => {
  ...
  const handleCloseReturn = async () => {
    ...
    try {
      setIsClosingReturn(true);
      const result = await returnService.closeReturn(returnItem.id);
      ...
      setIsClosingReturn(false);
    }
  };

  const handleCloseReturnModalClose = () => {
    setIsCloseModalOpen(false);
  };

  const handleCloseReturnClick = () => {
  ...
  if (!returnItem) {
    return (
      <div className="p-6">
        <div className="text-center">
          <p className="text-gray-500">{t('error.returnNotFound')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          <Btn
            variant="ghost"
            onClick={() => router.push(`/${locale}/dashboard/almacenes/devoluciones`)}
          ...
          <div>
            <h1 className="text-2xl font-bold" style={{ color: `rgb(var(--color-primary-800))` }}>
              {t('details.title', { name: returnItem.code })}
            </h1>
          ...
          {!returnItem.status && (
            <>
              ...
              <Btn
                ...
                onClick={handleCloseReturnClick}
                loading={isClosingReturn}
              >
                {t('actions.close')}
              </Btn>
            </>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm text-gray-900">{returnItem.code}</p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">{t('table.date')}:</span>
              <p className="text-sm text-gray-900">{formatDate(returnItem.date)}</p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">{t('table.status')}:</span>
              <span
                className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                  returnItem.status ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                }`}
              >
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm text-gray-900">{returnItem.sourceWarehouse.name}</p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">{t('table.code')}:</span>
              <p className="text-sm text-gray-900">{returnItem.sourceWarehouse.code}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="space-y-3">
            <div>
              <span className="text-sm font-medium text-gray-500">{t('table.name')}:</span>
              <p className="text-sm text-gray-900">{returnItem.targetProvider.name}</p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">{t('table.code')}:</span>
              <p className="text-sm text-gray-900">{returnItem.targetProvider.code}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        {loadingProducts ? (
          ...
          <ReturnProductsTable
            ...
            isReturnOpen={!returnItem.status}
          />
        )}
        
        {!returnItem.status && products.length === 0 && !loadingProducts && (
      ...
      {/* Modal de confirmación para cerrar devolución */}
      <CloseReturnModal
        isOpen={isCloseModalOpen}
        return={returnItem}
        onClose={handleCloseReturnModalClose}
        onConfirm={handleCloseReturn}
      />

      {/* Modal de resultado del cierre de devolución */}
      <ReturnCloseResultModal
