...
export default function AccountPayableDetailsPage() {
    ...
    const fetchAccount = async () => {
        try {
            setLoading(true);
            const data = await accountsPayableService.getAccountPayable(params.id as string);
            setAccount(data);
        } catch (error) {
            console.error('Error loading account payable:', error);
            toastService.error(t('messages.errorLoading'));
            router.push(`/${locale}/dashboard/finanzas/cuentas-por-pagar`);
        } finally {
            setLoading(false);
        }
    };
    ...
    if (!account) {
        return (
            <div className="p-6 text-center">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">{t('messages.errorLoading')}</h2>
                <Btn onClick={() => router.push(`/${locale}/dashboard/finanzas/cuentas-por-pagar`)}>
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    <Btn
                        variant="ghost"
                        onClick={() => router.push(`/${locale}/dashboard/finanzas/cuentas-por-pagar`)}
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white rounded-xl shadow-md overflow-hidden">
                        ...
                        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                            <InfoItem label={t('table.provider')} value={account.provider?.name || '-'} icon={<UserIcon className="h-4 w-4" />} />
                            ...
                            <InfoItem label={t('table.remainingAmount')} value={formatCurrency(account.remainingAmount)} valueClass="text-primary-500" isHighlight />
                        </div>
                        {account.notes && (
                                            ...
                                            <div className="mt-4 ml-14 p-3 bg-secondary-50 rounded-lg text-sm">
                                                ...
                                                {payment.createdByUser && (
                                                    <p className="text-[10px] uppercase tracking-wider font-bold text-secondary-400 flex items-center gap-1.5">
                                                        ...
                                                        <span className='text-gray-500'> {t('paymentHistory.registeredBy')}</span>   : {payment.createdByUser.name}
                                                    </p>
                                                )}
                                            </div>
                                        )}
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <p className="text-3xl text-secondary-500">{formatCurrency(account.remainingAmount)}</p>
                                </div>  
