...
export default function CashRegisterDetailsPage() {
    ...
    const handleBack = () => {
        router.push(`/${tenant}/${locale}/dashboard/ventas/cajas`);
    };
    ...
    useEffect(() => {
        const fetchCashRegister = async () => {
            ...
            try {
                setLoading(true);
                const response = await cashRegisterService.getCashRegister(Array.isArray(id) ? id[0] : id);
                setCurrentCashRegister(response);
            } catch {
    ...
    const handleInitializeCash = async (initialAmount: number) => {
        if (!currentCashRegister) return;
        ...
        try {
            ...
            const response = await cashRegisterService.getCashRegister(currentCashRegister.id);
            setCurrentCashRegister(response);
        } catch (error) {
        ...
        return (
            <div className="p-6">
                <div className="text-center py-12">
                    <p className="text-gray-500">{tCash('messages.errorFetchingInfo')}</p>
                </div>
            </div>
        );
    }
