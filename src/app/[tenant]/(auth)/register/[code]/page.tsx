...
export default function RegisterPage() {
  ...
  const referrerCodeFromUrl = (params.code as string) || '';
  
  const [formData, setFormData] = useState({
    name: '', companyName: '', email: '',
    password: '', password_confirmation: '', referrer_code: referrerCodeFromUrl,
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // No permitir cambiar el código de referido si viene de la URL
    if (e.target.name === 'referrer_code' && referrerCodeFromUrl) {
      return;
    }
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input 
                ...
                type="text" 
                maxLength={20} 
                className={`${inputCls} uppercase ${referrerCodeFromUrl ? 'bg-gray-100 cursor-not-allowed' : ''}`} 
                style={inputStyle(false)} 
                value={formData.referrer_code} 
                onChange={handleChange}
                disabled={!!referrerCodeFromUrl}
